...
const generateMockFeedback = (response: string, question: Question) => {
  ...
  const hasKeywords = modelAnswer.split(" ").some((word: string) => response.toLowerCase().includes(word))
...
export default function ProgressBoostClient() {
  ...
  const saveAnswer = async (payload: { response_text: string; autoScore?: ScoreType | null }) => {
    ...
    if (userType === "revisionAI" && !payload.autoScore) {
      // Simulate delay + generate feedback
      await new Promise((resolve) => setTimeout(resolve, 1500))
      const mockFeedback = generateMockFeedback(payload.response_text, q)
  ...
  const handleMatchingAnswer = async (selections: Record<string, string[]>) => {
    if (currentIndex >= questions.length) return
    const q = questions[currentIndex]
    const isCorrect = q.pairs?.every(pair =>
      selections[pair.statement]?.includes(pair.match)
    ) || false
    setIsSubmitting(true)
    await saveAnswer({ response_text: JSON.stringify(selections), autoScore: isCorrect ? "green" : "red" })
    setIsSubmitting(false)
    await afterAnswered()
  }
  ...
  const handleSkip = async () => {
    ...
    if (!item) return
    // Mark as skipped in DB
    const { error } = await supabase.from("progressboost_items").update({ status: "skipped" }).eq("id", item.item_id)
    if (error) {
      console.error(error)
      toast.error("Failed to skip: " + explain(error))
      return
  ...
  const renderAnswerReview = () => {
    ...
    if (q.type === "fill-in-the-blank") {
      let selected: number[] = []
      try {
        selected = JSON.parse(answerPayload || "[]")
      } catch {}
      const selectedOptions = selected.map((i) => q.options?.[i])
      const model = Array.isArray(q.model_answer) ? q.model_answer : []
      const orderImportant = q.order_important
        ...
        <div className="space-y-4">
          ...
          <div className="p-3 bg-emerald-50 rounded border border-emerald-100">
            ...
            <ul className="list-disc pl-5">
              {q.pairs?.map((p, i) => (
                <li key={i}>
                  <strong>{p.statement}:</strong> {p.match}
                </li>
              ))}
            </ul>
          </div>
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-3xl">
      ...
      <Card className="mb-6">
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-3 gap-2 text-sm">
            <div className="bg-green-50 p-2 rounded">
              Easy:{" "}
              {getNum(progressMaps.done, "low:new") +
                getNum(progressMaps.done, "low:mid") +
                getNum(progressMaps.done, "low:old")}{" "}
              /
              {getNum(progressMaps.target, "low:new") +
                getNum(progressMaps.target, "low:mid") +
                getNum(progressMaps.target, "low:old")}
            </div>
            <div className="bg-yellow-50 p-2 rounded">
              Medium:{" "}
              {getNum(progressMaps.done, "medium:new") +
                getNum(progressMaps.done, "medium:mid") +
                getNum(progressMaps.done, "medium:old")}{" "}
              /
              {getNum(progressMaps.target, "medium:new") +
                getNum(progressMaps.target, "medium:mid") +
                getNum(progressMaps.target, "medium:old")}
            </div>
            <div className="bg-red-50 p-2 rounded">
              Hard:{" "}
              {getNum(progressMaps.done, "high:new") +
                getNum(progressMaps.done, "high:mid") +
                getNum(progressMaps.done, "high:old")}{" "}
              /
              {getNum(progressMaps.target, "high:new") +
                getNum(progressMaps.target, "high:mid") +
                getNum(progressMaps.target, "high:old")}
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      {completed && (
        <Card className="border-green-300 bg-green-50 mb-6">
          ...
          <CardContent>
            ...
            Awesome! You&apos;ve hit all this week&apos;s targets. Come back next week for a new Boost.
            </p>
          </CardContent>
        </Card>
      )}
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Badge className="capitalize" variant="outline">
                  {String(questions[currentIndex].difficulty)}
                </Badge>
                {!answerId && (
                  <Button
                    variant="outline"
          ...
          <CardContent>
            ...
            {answerId ? (
              <>
                ...
                {/* For auto-marked types, show their score banner */}
                {autoScore !== null ? (
                  <div className="mt-4">
                    {answer ? <FeedbackDisplay answer={answer} /> : null}
                  </div>
                ) : (
                  /* For manual assessment types, check user type */
                  <div className="mt-4">
                    {/* For revisionAI users, show AI feedback */}
                    {userType === "revisionAI" ? (
                      answer ? <FeedbackDisplay answer={answer} /> : null
                    ) : /* For other users, show self-assessment or feedback based on completion */
                    !selfAssessmentScore ? (
                      <SelfAssessment onSelectScore={handleSelfAssess} />
                    ) : answer ? (
                ...
                <div className="mt-6 flex flex-col sm:flex-row gap-3">
                  <Button
                    ...
                    disabled={autoScore === null && userType !== "revisionAI" && !selfAssessmentScore}
                  >
                    <RefreshCw className="mr-2 h-4 w-4" /> Try Another Question
                  </Button>
                  <Link href="/progress">
              ...
              questions[currentIndex].type === "algorithm" ||
              questions[currentIndex].type === "sql" ? (
              <CodeQuestion onSubmit={handleSubmitAnswer} disabled={isSubmitting} />
            ) : questions[currentIndex].type === "matching" ? (
              <MatchingQuestion
                question={questions[currentIndex]}
      ...
      {!completed && !loading && questions.length === 0 && <p>No questions needed right now. ✅ You&apos;re up to date!</p>}
    </div>
  )
}
