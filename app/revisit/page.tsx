...
export default function RevisitPage() {
  ...
  // state for counts
  const [counts, setCounts] = useState<RevisitCounts>({ total: 0, green: 0, amber: 0, red: 0 })
  const [countsLoading, setCountsLoading] = useState(false)

  // ====== NEW: reset paging when filters change (or when tab changes) ======
  useEffect(() => {
    setAllAnswers([])
    setQuestions({})
    setPage(0)
    setHasMore(true)
  }, [selectedTopics, typeParam, difficultyParam, activeTab])

  // Memoize filtered answers (topic)
  const filteredAnswers = useMemo(() => {
    if (allAnswers.length === 0 || topics.length === 0) return allAnswers
    return selectedTopics.length > 0
      ? allAnswers.filter((answer) => {
  ...
  // ====== INITIAL BOOTSTRAP: auth + topics (no data page here) ======
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      ...
      const { data, error } = await supabase.rpc('get_revisit_counts_v1', {
        p_user: user.id,
        p_topic_slugs: topicsArg,
        p_type: typeArg,
        p_difficulty: difficultyArg,
      })
  ...
  // ====== PAGE LOADER: offset-based (does NOT depend on activeTab) ======
  useEffect(() => {
    if (!user) return
    let aborted = false
    ...
    const loadPage = async () => {
      ...
      const { data: rows, error } = await supabase.rpc('get_revisit_attempts_v2', {
        p_user: user.id,
        p_topic_slugs: topicsArg,
        p_type: typeArg,
        p_difficulty: difficultyArg,
      ...
      if (error) {
        console.error('page rpc error', error)
      ...
      rows?.forEach((row: RevisitRow) => {
        newAnswers.push({
          ...
          ai_feedback: null,
          score: (row.student_score ?? "amber") as ScoreType,
          submitted_at: row.submitted_at,
          self_assessed: false,
        })

        const q = row.question
        newQuestions[row.question_id] = {
          id: q.id,
          type: q.type as Question['type'],
          difficulty: (q.difficulty as Question['difficulty']) ?? 'medium',
          topic: q.topic || '',
      ...
      setAllAnswers((prev) => {
        ...
        const deduped = [...prev, ...newAnswers.filter((a) => !seen.has(a.id))]
        return deduped
      })
  ...
  }, [user, page, selectedTopics, typeParam, difficultyParam])

  // ====== INTERSECTION OBSERVER: bump page when sentinel enters view ======
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (first.isIntersecting && !isPageLoading) {
          setPage((p) => p + 1)
        }
      },
      { rootMargin: '300px' }
  ...
  // Filter answers by score/type/difficulty
  const filteredAnswersByScoreAndType = useMemo(() => {
    return filteredAnswers
      .filter((answer) => activeTab === "all" || answer.score === activeTab)
      .filter((answer) => {
  ...
  // Group answers by topic
  const answersByTopic = useMemo(() => {
    return filteredAnswersByScoreAndType.reduce((acc, answer) => {
      const question = questions[answer.question_id]
      if (!question) return acc
      const topicSlug = topics.find((t) => t.id === question.topic)?.slug || "unknown"
      ;(acc[topicSlug] ||= []).push(answer)
        ...
        <div className="w-full grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
          ...
          >
            <div className="text-center w-full">
              ...
              <div className="text-xs text-muted-foreground">{countsLoading ? '…' : `${counts.total} total`}</div>
            </div>
          </Button>
          <Button
            variant="outline"
          ...
          >
            <div className="text-center w-full">
              ...
              <div className="text-xs">{countsLoading ? '…' : `${counts.green} questions`}</div>
            </div>
          </Button>
          <Button
            variant="outline"
          ...
          >
            <div className="text-center w-full">
              ...
              <div className="text-xs">{countsLoading ? '…' : `${counts.amber} questions`}</div>
            </div>
          </Button>
          <Button
            variant="outline"
          ...
          >
            <div className="text-center w-full">
              ...
              <div className="text-xs">{countsLoading ? '…' : `${counts.red} questions`}</div>
            </div>
          </Button>
        </div>
        ...
        {["all", "green", "amber", "red"].map((tab) => (
          activeTab === tab && (
            <div key={tab} className="space-y-6">
                            ...
                            <CardContent className="space-y-6">
                                    ...
                                    {question.type === "fill-in-the-blank" ? (
                                        ...
                                        {(() => {
                                          try {
                                            ...
                                            const modelAnswer = Array.isArray(question.model_answer)
                                              ? (question.model_answer as string[])
                                            ...
                                            return (
                                              <div className="space-y-2">
                                                {selectedOptions.map((option, i) => {
                                                  const isOptionCorrect = question.order_important
                                                    ? option === modelAnswer[i]
                                                    : option ? modelAnswer.includes(String(option)) : false
                                                  return (
                                                    <div key={i} className={`flex items-center gap-2 ${isOptionCorrect ? "text-emerald-600" : "text-red-600"}`}>
                                                      {option || "No answer selected"}
                                                      {isOptionCorrect ? (
                                      ...
                                      <div className="overflow-x-auto">
                                        <table className="w-full border-collapse text-sm">
                                          <thead>
                                            <tr>
                                              <th className="border border-gray-200 p-2 text-left bg-gray-50 text-gray-700">Statement</th>
                                              <th className="border border-gray-200 p-2 text-left bg-gray-50 text-gray-700">Your Match</th>
                                              <th className="border border-gray-200 p-2 text-center bg-gray-50 text-gray-700 w-12">✓</th>
                                            </tr>
                                          </thead>
                                          <tbody>
                                            {question.pairs?.map((pair, index) => {
                                              ...
                                              return (
                                                <tr key={index} className={isCorrect ? "bg-emerald-50" : "bg-red-50"}>
                                                  <td className="border border-gray-200 p-2 text-gray-700">{pair.statement}</td>
                                                  <td className="border border-gray-200 p-2 text-gray-700">{userMatches.join(", ") || "No match selected"}</td>
                                                  <td className="border border-gray-200 p-2 text-center">
                                                    <div className="flex justify-center">
                                                      {isCorrect ? (
                                                        <CheckCircle className="h-3 w-3 text-emerald-600" />
                                      ...
                                      (() => {
                                        ...
                                        return (
                                          <div className="text-center">
                                            <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${isCorrect ? "bg-emerald-100 text-emerald-800" : "bg-red-100 text-red-800"}`}>
                                              {userAnswer === "true" ? "True" : "False"}
                                              {isCorrect ? (
                                                <CheckCircle className="h-4 w-4 ml-2" />
                                              ) : (
                                      ...
                                      <div className="space-y-2">
                                        {question.options?.map((option, index) => {
                                          ...
                                          if (!isCorrect) return null
                                          return (
                                            <div key={index} className="p-2 rounded border bg-emerald-100 border-emerald-300 text-emerald-800">
                                              <div className="flex items-center justify-between">
                                                <span>{option}</span>
                                                <CheckCircle className="h-4 w-4" />
                                              </div>
                                    ...
                                    {question.type === "fill-in-the-blank" ? (
                                      <div className="space-y-2">
                                        {Array.isArray(question.model_answer) ? (
                                          question.order_important ? (
                                            <ol className="list-decimal pl-4 space-y-1">
                                              {(question.model_answer as string[]).map((ans, idx) => (
                                                <li key={idx} className="text-emerald-700">{ans}</li>
                                              ))}
                                            </ol>
                                          ) : (
                                            <ul className="list-disc pl-4 space-y-1">
                                          ...
                                          <span className="text-emerald-700">{String(question.model_answer)}</span>
                                        )}
                                      </div>
                                    ) : question.type === "matching" ? (
                                      <div className="overflow-x-auto">
                                        <table className="w-full border-collapse text-sm">
                                          <thead>
                                            <tr>
                                              <th className="border border-emerald-200 p-2 text-left bg-emerald-50 text-emerald-700">Statement</th>
                                              <th className="border border-emerald-200 p-2 text-left bg-emerald-50 text-emerald-700">Correct Match</th>
                                              <th className="border border-emerald-200 p-2 text-center bg-emerald-50 text-emerald-700 w-12">✓</th>
                                            </tr>
                                          </thead>
                                          <tbody>
                                            {question.pairs?.map((pair, index) => (
                                              <tr key={index} className="bg-emerald-25">
                                                <td className="border border-emerald-200 p-2 text-emerald-700">{pair.statement}</td>
                                                <td className="border border-emerald-200 p-2 text-emerald-700">{pair.match}</td>
                                                <td className="border border-emerald-200 p-2 text-center">
                                                  <CheckCircle className="h-3 w-3 text-emerald-600 mx-auto" />
                                                </td>
                                              </tr>
                                      ...
                                      <div className="text-center">
                                        <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-emerald-100 text-emerald-800">
                                          {String(question.model_answer).trim().toLowerCase() === "true" ? "True" : "False"}
                                          <CheckCircle className="h-4 w-4 ml-2" />
                                        </span>
                                      </div>
                                    ) : question.type === "multiple-choice" ? (
                                      ...
                                      <div className="space-y-3">
                                        {question.type === "code" && (
                                          <div>
                                            <h4 className="text-sm font-medium mb-2 text-emerald-800">Pseudocode:</h4>
                                            <pre className="whitespace-pre-wrap font-sans text-sm text-emerald-700">{String(question.model_answer)}</pre>
                                          </div>
                                        )}
                                        {question.type !== "code" && (
                                          <pre className="whitespace-pre-wrap font-sans text-sm text-emerald-700">{String(question.model_answer)}</pre>
                                        ...
                                        {question.model_answer_code && (
                                          <div className="border-t border-emerald-200 pt-3">
                                            <h4 className="text-sm font-medium mb-2 text-emerald-800">{question.language}:</h4>
                                            <pre className="whitespace-pre-wrap font-mono text-sm text-emerald-700 bg-emerald-25 p-2 rounded">{question.model_answer_code}</pre>
                                          </div>
                                        )}
                                      </div>
                                    )}
                              ...
                              {question.explanation && (
                                <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4">
                                  ...
                                  <div className="bg-white border border-indigo-100 rounded-md p-3">
                                    <p className="whitespace-pre-wrap text-sm text-indigo-700">{question.explanation}</p>
                                  </div>
                                </div>
                              )}
              ...
              {/* ====== NEW: sentinel + loading states for infinite scroll ====== */}
              <div ref={sentinelRef} className="h-8" />
              {isPageLoading && (
                <div className="flex items-center justify-center py-6 text-sm text-gray-500">Loading more…</div>
              )}
              {!hasMore && allAnswers.length > 0 && (
                <div className="flex items-center justify-center py-6 text-sm text-gray-500">You’re all caught up.</div>
              )}
            </div>
          )
        ))}
          ...
          <DialogContent className="sm:max-w-[425px]">
            ...
            <div className="py-6 px-1">
              <div className="space-y-3">
                ...
                <Input
                  ...
                  value={deleteConfirmation}
                  onChange={(e) => setDeleteConfirmation(e.target.value)}
                  className={`w-full ${deleteConfirmation && deleteConfirmation !== "delete" ? "border-red-300 focus-visible:ring-red-500" : ""}`}
                />
                {deleteConfirmation && deleteConfirmation !== "delete" && (
                  <p className="text-sm text-red-500 mt-1">Please type &quot;delete&quot; exactly to confirm</p>
                )}
            ...
            <DialogFooter className="flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2">
              ...
              <Button
                ...
                onClick={handleDeleteConfirm}
                disabled={deleteConfirmation !== "delete" || isDeleting}
                className={`flex items-center gap-2 ${deleteConfirmation === "delete" ? "bg-red-600 hover:bg-red-700 text-white" : "bg-red-100 text-red-400 cursor-not-allowed"}`}
              >
                {isDeleting ? (
                  <>
                    <div className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent" />
